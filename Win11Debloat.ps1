...
# Ask user about Teams and OneDrive
if (-not $Silent) {
    $appslistPath = "$PSScriptRoot/Appslist.txt"
    $appslistContent = Get-Content -Path $appslistPath

    # Ask about Teams
    $uninstallTeams = Read-Host -Prompt "Do you want to uninstall Microsoft Teams? (y/n)"
    if ($uninstallTeams -eq 'n') {
        # Keep Teams, so comment it out
        for ($i = 0; $i -lt $appslistContent.Length; $i++) {
            if (($appslistContent[$i] -like '*MicrosoftTeams*') -or ($appslistContent[$i] -like '*MSTeams*')) {
                if (-not $appslistContent[$i].Trim().StartsWith('#')) {
                    $appslistContent[$i] = '#' + $appslistContent[$i]
                }
            }
        }
        Write-Output "Microsoft Teams will not be uninstalled."
    } elseif ($uninstallTeams -eq 'y') {
        # Uninstall Teams, so uncomment it
        for ($i = 0; $i -lt $appslistContent.Length; $i++) {
            if (($appslistContent[$i] -like '*MicrosoftTeams*') -or ($appslistContent[$i] -like '*MSTeams*')) {
                $appslistContent[$i] = $appslistContent[$i].TrimStart('#')
            }
        }
        Write-Output "Microsoft Teams will be uninstalled."
    }

    # Ask about OneDrive
    $uninstallOneDrive = Read-Host -Prompt "Do you want to uninstall OneDrive? (y/n)"
    if ($uninstallOneDrive -eq 'n') {
        # Keep OneDrive, so comment it out
        for ($i = 0; $i -lt $appslistContent.Length; $i++) {
            if ($appslistContent[$i] -like '*Microsoft.OneDrive*') {
                 if (-not $appslistContent[$i].Trim().StartsWith('#')) {
                    $appslistContent[$i] = '#' + $appslistContent[$i]
                }
            }
        }
        Write-Output "OneDrive will not be uninstalled."
    } elseif ($uninstallOneDrive -eq 'y') {
        # Uninstall OneDrive, so uncomment it
        for ($i = 0; $i -lt $appslistContent.Length; $i++) {
            if ($appslistContent[$i] -like '*Microsoft.OneDrive*') {
                $appslistContent[$i] = $appslistContent[$i].TrimStart('#')
            }
        }
        Write-Output "OneDrive will be uninstalled."
    }

    # Save changes to Appslist.txt
    Set-Content -Path $appslistPath -Value $appslistContent
    Write-Output ""
}
